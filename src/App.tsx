...
import { Search, Plus, User, LogIn, LogOut, ChevronRight, MapPin, Clock, Phone, Camera, X, Filter, ChevronLeft, Trash2, Wrench, PhoneCall, Star, Check, Shield, AlertCircle, Menu, Info, Mail, Eye, EyeOff, Globe, Edit, Calendar } from 'lucide-react';
...
function SponsorList({ sponsors, isDarkMode, onUpdate, user, onEdit }: { 
  ...
  const updateWeightage = async (id: string) => {
    try {
      const res = await fetch(`/api/sponsors/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ weightage: newWeightage, adminEmail: user?.email })
      });
      if (res.ok) {
          ...
          return (
            <div key={s.id} className={`p-4 rounded-2xl border shadow-sm space-y-3 ${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {editingWeightage === s.id ? (
                  <div className="flex items-center gap-2 w-full">
                    <input
                      type="number"
                      className={`flex-1 p-2 rounded-lg text-xs outline-none ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-gray-50 text-gray-900'}`}
                      value={newWeightage}
                      onChange={e => setNewWeightage(parseInt(e.target.value) || 0)}
                    />
                    <button onClick={() => updateWeightage(s.id)} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg font-bold">Save</button>
                    <button onClick={() => setEditingWeightage(null)} className="text-xs text-gray-500 px-2 py-1.5">Cancel</button>
                  </div>
                ) : (
                  <button 
                    onClick={() => {
                      setEditingWeightage(s.id);
                      setNewWeightage(s.weightage || 0);
                    }}
