...
import { Search, Plus, User, LogIn, LogOut, ChevronRight, MapPin, Clock, Phone, Camera, X, Filter, ChevronLeft, Trash2, Wrench, PhoneCall, Star, Check, Shield, AlertCircle, Menu, Info, Mail, Eye, EyeOff, Globe, Edit, Calendar, ChevronUp, ChevronDown } from 'lucide-react';
...
function SponsorList({ sponsors, isDarkMode, onUpdate, user, onEdit }: { 
  ...
  const moveSponsor = async (id: string, currentRank: number, direction: 'up' | 'down') => {
    const newRank = direction === 'up' ? currentRank - 1 : currentRank + 1;
    if (newRank < 1 || newRank > sponsors.length) return;

    try {
      const res = await fetch(`/api/sponsors/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rank: newRank, adminEmail: user?.email })
      });
      if (res.ok) onUpdate();
    } catch (err) {
      console.error(err);
          ...
          return (
            <div key={s.id} className={`p-4 rounded-2xl border shadow-sm space-y-3 ${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold text-purple-600 dark:text-purple-400">Rank: {s.rank}</span>
                  <div className="flex flex-col">
                    <button 
                      disabled={s.rank === 1}
                      onClick={() => moveSponsor(s.id, s.rank, 'up')}
                      className="text-gray-400 hover:text-blue-500 disabled:opacity-30"
                    >
                      <ChevronUp size={14} />
                    </button>
                    <button 
                      disabled={s.rank === sponsors.length}
                      onClick={() => moveSponsor(s.id, s.rank, 'down')}
                      className="text-gray-400 hover:text-blue-500 disabled:opacity-30"
                    >
                      <ChevronDown size={14} />
                    </button>
