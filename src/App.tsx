        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className={`${isDarkMode ? 'bg-gray-900 border-gray-800 hover:bg-gray-800' : 'bg-white border-blue-100 hover:bg-blue-50'} p-6 rounded-2xl shadow-sm border flex flex-col items-center gap-3 transition-colors`}
                >
                ...
                <div className="grid grid-cols-2 gap-4">
                  {topCategories.length > 0 ? (
                    topCategories.map((cat, idx) => (
                      ...
                      >
                        {/* 2x2 Image Grid */}
                        <div className={`aspect-square grid grid-cols-2 gap-0.5 p-0.5 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                          {[0, 1, 2, 3].map(i => (
                            <div key={i} className={`${isDarkMode ? 'bg-gray-900' : 'bg-white'} overflow-hidden flex items-center justify-center`}>
                              {cat.thumbnails && cat.thumbnails[i] ? (
                                ...
                                <div className={`w-full h-full flex items-center justify-center p-0.5 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
                                  <img 
          ...
          {activeTab === 'find' && (
            ...
            >
              ...
              <div className={`${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} p-4 rounded-2xl shadow-sm space-y-3 border transition-colors`}>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                  <input
                    type="text"
                    placeholder="Search services..."
                    className={`w-full pl-10 pr-4 py-3 border-none rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-colors ${isDarkMode ? 'bg-gray-800 text-white placeholder-gray-500' : 'bg-gray-50 text-gray-900'}`}
                    value={searchQuery}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select
                    className={`p-2 rounded-lg text-sm outline-none border-none focus:ring-1 focus:ring-blue-500 transition-colors ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-gray-50 text-gray-900'}`}
                    value={filterState}
                    onChange={(e) => {
                  ...
                  <select
                    className={`p-2 rounded-lg text-sm outline-none border-none focus:ring-1 focus:ring-blue-500 transition-colors disabled:opacity-50 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-gray-50 text-gray-900'}`}
                    value={filterTown}
                ...
                <div className={`${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} p-4 rounded-2xl shadow-sm space-y-3 border transition-colors`}>
                  <div className="flex justify-between items-center">
                    <label className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Rating Range</label>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isDarkMode ? 'text-blue-400 bg-blue-900/30' : 'text-blue-600 bg-blue-50'}`}>
                      {minRating} - {maxRating} Stars
                    </span>
                  </div>
                  <div className="relative h-6 flex items-center">
                    <div className={`absolute w-full h-1.5 rounded-full ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`} />
                    <div 
                  ...
                  <div className="flex justify-between px-1">
                    {[0, 1, 2, 3, 4, 5].map(tick => (
                      <span key={tick} className={`text-[10px] font-medium ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>{tick}</span>
                    ))}
                  </div>
                </div>
                                ...
                                <tbody className={`divide-y divide-gray-100 dark:divide-gray-700 ${isDarkMode ? '' : 'bg-white'}`}>
                                  {adminUsers.map((u, i) => (
                                    <tr key={u.id} className={`${isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50 bg-white'} transition-colors`}>
                                      ...
                                      <td className={`p-3 font-bold ${isDarkMode ? 'text-black' : 'text-gray-900'}`}>{u.firstName} {u.lastName}</td>
                                      <td className={`p-3 ${isDarkMode ? 'text-black' : 'text-gray-700'}`}>{u.mobileNumber}</td>
                                      <td className={`p-3 ${isDarkMode ? 'text-black' : 'text-gray-700'}`}>{u.email}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
