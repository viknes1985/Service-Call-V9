  ...
  app.get("/api/services", async (req, res) => {
    ...
    try {
      ...
      if (filter.status === 'approved' && !createdBy && isAdmin !== 'true') {
        ...
        // 1. Get 8 regular non-sponsored services
        ...
        // 2. Get 1 sponsored service (Type B)
        ...
        // 3. Get 1 sponsor advertisement (Type A)
        const activeSponsors = await Sponsor.find({ isEnabled: true });
        ...
        // Split back
        const enrichedNonSponsored = enriched.filter(s => !s.isSponsored).slice(0, 8);
        ...
        const totalPages = Math.ceil(totalNonSponsored / 8);
