  ...
  app.get("/api/services", async (req, res) => {
    ...
    try {
      ...
      if (filter.status === 'approved' && !createdBy) {
        ...
        while (sponsoredIdx < sponsored.length && tempResult.length < 2) {
          tempResult.push(sponsored[sponsoredIdx++]);
        }

        // Interleave the rest: every 5 non-sponsored, add 1 sponsored
        for (let i = 0; i < nonSponsored.length; i++) {
          tempResult.push(nonSponsored[i]);
        ...
        // Now interleave Sponsor objects (from Sponsor collection) every 8 items
        const activeSponsors = await Sponsor.find({ isEnabled: true }).sort({ createdAt: 1 });
        let withSponsors: any[] = [];
        if (activeSponsors.length > 0) {
          let sponsorIdx = 0;
          for (let i = 0; i < tempResult.length; i++) {
            withSponsors.push({ type: 'service', data: tempResult[i] });
            if ((i + 1) % 8 === 0) {
              withSponsors.push({ type: 'sponsor', data: activeSponsors[sponsorIdx % activeSponsors.length] });
              sponsorIdx++;
            }
          }
          result = withSponsors;
        } else {
          result = tempResult.map(s => ({ type: 'service', data: s }));
        }
      } else {
        result = enriched.map(s => ({ type: 'service', data: s }));
      }

      // Apply pagination to the final mixed result
