...
async function startServer() {
  ...
  app.get("/api/services", async (req, res) => {
    ...
    try {
      const services = await Service.find(filter).sort({ createdAt: -1, _id: -1 });
      ...
      if (filter.status === 'approved' && !createdBy && isAdmin !== 'true') {
        ...
        // 1. One deterministic sponsored service (if any)
        ...
        // 3. Fill the rest with non-sponsored services (already sorted by createdAt: -1)
        ...
        // REMOVED SHUFFLE to maintain sorting
