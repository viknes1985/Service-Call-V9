...
// Migration for multiple towns and contact numbers
(async () => {
  try {
    const servicesToMigrate = await Service.find({
      $or: [
        ...
        { contactNumbers: { $eq: null } },
        { contactNumbers: { $size: 0 } }
      ]
    });
    ...
    for (const s of servicesToMigrate) {
      let changed = false;
      const sObj = s as any;
      if ((!sObj.towns || sObj.towns.length === 0) && sObj.town) {
        sObj.towns = [sObj.town];
        changed = true;
      }
      if ((!sObj.contactNumbers || sObj.contactNumbers.length === 0) && sObj.contactNumber) {
        sObj.contactNumbers = [sObj.contactNumber];
        changed = true;
      }
      if (changed) await s.save();
    }
    if (servicesToMigrate.length > 0) {
      console.log(`✅ Migrated ${servicesToMigrate.length} services to support multiple towns/numbers`);
    }
  } catch (e) {
    console.warn("Migration notice (expected if running first time):", e);
  }
})();
...
async function startServer() {
  ...
  app.get("/api/services", async (req, res) => {
    ...
    try {
      ...
      if (filter.status === 'approved' && !createdBy && isAdmin !== 'true') {
        ...
        // 5. Select 1 sponsor advertisement (Type A) - Based on rank order
        let sponsorAd: any = null;
        if (hasSponsorAd) {
          const sortedSponsors = [...activeSponsors].sort((a, b) => (a.rank || 0) - (b.rank || 0));
          const index = (currentPage - 1) % sortedSponsors.length;
          sponsorAd = sortedSponsors[index];
        }
        ...
        // Build the page items in specific order:
        // 1st: Regular 1
        // 2nd: Type B
        // 3rd-9th: Regular 2-8
        // 10th: Type A
        ...
        if (enrichedNonSponsored.length > 0) {
          pageItems.push({ type: 'service', data: enrichedNonSponsored[0] });
        }
        
        if (enrichedSponsored) {
          pageItems.push({ type: 'service', data: enrichedSponsored });
        }
        
        for (let i = 1; i < enrichedNonSponsored.length; i++) {
          pageItems.push({ type: 'service', data: enrichedNonSponsored[i] });
        }
        
        if (sponsorAd) {
          pageItems.push({ type: 'sponsor', data: sponsorAd });
        }
            ...
            html: `
              <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                ...
                <div style="background: #f9fafb; padding: 15px; border-radius: 8px;">
                  ...
                  <p><strong>Location:</strong> ${(req.body.towns || [req.body.town]).join(', ')}, ${req.body.state}</p>
                </div>
  ...
  app.get("/api/sponsors", async (req, res) => {
    try {
      const sponsors = await Sponsor.find().sort({ rank: 1 });
  ...
  app.post("/api/sponsors", async (req, res) => {
    ...
    try {
      ...
      // Find max rank and assign max + 1
      const maxRankSponsor = await Sponsor.findOne().sort({ rank: -1 });
      const nextRank = maxRankSponsor ? (maxRankSponsor.rank || 0) + 1 : 1;
  ...
  app.put("/api/sponsors/reorder", async (req, res) => {
    const { adminEmail, sponsorRanks } = req.body; // sponsorRanks: { id: string, rank: number }[]
    if (!isAdminEmail(adminEmail)) {
      return res.status(403).json({ error: "Unauthorized" });
    }

    try {
      for (const item of sponsorRanks) {
        await Sponsor.findByIdAndUpdate(item.id, { rank: item.rank });
      }
      res.json({ success: true });
    } catch (err: any) {
      res.status(400).json({ error: err.message });
  ...
  app.patch("/api/sponsors/:id", async (req, res) => {
    const { id } = req.params;
    const { adminEmail, isEnabled, durationDays, name, phone, email, photoUrls, rank } = req.body;
    if (!isAdminEmail(adminEmail)) {
      return res.status(403).json({ error: "Unauthorized" });
    }
    ...
    try {
      ...
      if (rank !== undefined) {
        const oldRank = sponsor.rank;
        const newRank = rank;
        
        if (oldRank !== newRank) {
          // Find the sponsor at the target rank and swap
          const targetSponsor = await Sponsor.findOne({ rank: newRank });
          if (targetSponsor) {
            targetSponsor.rank = oldRank;
  ...
  app.delete("/api/sponsors/:id", async (req, res) => {
    ...
    try {
      ...
      // Re-index ranks for remaining sponsors
      const remainingSponsors = await Sponsor.find().sort({ rank: 1 });
